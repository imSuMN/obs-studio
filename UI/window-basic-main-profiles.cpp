				   ...
				   std::string &dirName)
{
	...
	ret = GetConfigPath(path, sizeof(path), "ping-studio/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
static bool CopyProfile(const char *fromPartial, const char *to)
{
	...
	ret = GetConfigPath(dir, sizeof(dir), "ping-studio/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
void OBSBasic::DeleteProfile(const char *profileName, const char *profileDir)
{
	...
	int ret = GetConfigPath(basePath, 512, "ping-studio/basic/profiles");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return;
	}
...
void OBSBasic::on_actionImportProfile_triggered()
{
	...
	int ret = GetConfigPath(path, 512, "ping-studio/basic/profiles/");
	if (ret <= 0) {
